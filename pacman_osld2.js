...
let baseTileSize = 40;
let tileSize = baseTileSize * 0.85;
...
function updateMousePosition(clientX, clientY) {
  mouse.x = Math.min(cols - 1, Math.max(0, Math.floor(clientX / tileSize)));
  mouse.y = Math.min(rows - 1, Math.max(0, Math.floor(clientY / tileSize)));
}
...
function updatePacman() {
  ...
  if (!pacman.moving && now - lastMouseUpdate > 300) {
    const target = { x: mouse.x, y: mouse.y };
    if (target.x !== pacman.lastGoal.x || target.y !== pacman.lastGoal.y) {
...
function resizeCanvasAndMaze() {
  ...
  particles = [];
  const numParticles = Math.max(30, Math.floor((width * height) / 30000));
